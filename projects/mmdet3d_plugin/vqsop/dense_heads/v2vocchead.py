...
from projects.mmdet3d_plugin.vqsop.loss.loss_utils import multiscale_supervision, geo_scal_loss, sem_scal_loss
...
from projects.mmdet3d_plugin.vqsop.modules.codebook import UMGMQuantizer #import codebook module
from projects.mmdet3d_plugin.vqsop.modules.asr import ASR
from projects.mmdet3d_plugin.vqsop.modules.feature_restore import AdaptiveMultiScaleRefinement
...
#---------------------------------------
        ...
        if codebook is not None:
            self.seg_num = codebook['seg_num']
            self.dict_size = [codebook['dict_size']] * 3
            self.stage = codebook['stage']
            self.codebook_mask_threshold = codebook.get('codebook_mask_threshold', 0.8)
        else:
            self.seg_num = 6
            self.dict_size = [256] * 3  # default to 256 for all stages
            self.stage = 1
            self.codebook_mask_threshold = 0.8
    ...
    def forward(self, mcar_feats, img_metas, is_test=False):
        ##mcar_feats:(car_num,scale,B,cam,C,H,W)
        car_num = len(mcar_feats)
        volume_embed = []
        confidences = []
        for car in range(car_num):
            mlvl_feats = mcar_feats[car]#(scale,B,cam,C,H,W)
            bs, num_cam, _, _, _ = mlvl_feats[0].shape
            dtype = mlvl_feats[0].dtype

            volume_queries = self.volume_embedding[0].weight.to(dtype)#(volume_h*volume_w*volume_z,C)
            ...
            volume_embed_i = self.transformer[0](
                    mlvl_feats,#(scale,B,cam,C,H,W)
                    volume_queries,#(volume_h*volume_w*volume_z,C)
                    volume_h=volume_h,
                    volume_w=volume_w,
                    volume_z=volume_z,
        ...
        heter_feature_3d=torch.stack(volume_embed)
        # === Codebook logic ===
        carnum, bs, C, Z, H, W = heter_feature_3d.shape
        """
        carnum = number of agents

        C = channels

        Z, H, W = spatial size (feature map dimensions)
        """
        

        flattened = heter_feature_3d.permute(0,1,3,4,5,2).contiguous().view(-1, C)#(carnum*bs*Z*H*W, C)
        if not is_test:
            quantized, _, _, codebook_loss = self.codebook(flattened)
            if self.stage == 3:
                codebook_mask = torch.stack(confidences)
                codebook_mask = (codebook_mask > self.codebook_mask_threshold).float().detach()
                codebook_mask = codebook_mask.permute(0, 1, 3, 4, 5, 2).contiguous().view(-1, 1)

                voxel_codebook_loss = F.mse_loss(
                    quantized,
                    flattened.detach(),
                    reduction='none',
                ).mean(dim=1, keepdim=True)
                codebook_loss = (voxel_codebook_loss * codebook_mask).sum()
                codebook_loss = codebook_loss / codebook_mask.sum().clamp_min(1.0)
        else:
            with torch.no_grad():
        ...
        quantized = quantized.view(carnum,bs, Z, H, W, C).permute(0, 1, 5, 2, 3, 4).contiguous()
        heter_feature_3d = quantized
        
        
        features = volume_embed
        features = heter_feature_3d
        ...
        for b in range(bs):
            ego_feature = volume_embed[0][b,...].unsqueeze(0) #(1,C,Z,H,W)
            fuse_features = []
            ...
            for neighbor in range(1, car_num):
                ...
                if img_metas[0]["vehicle_id"][0] == img_metas[0]["vehicle_id"][neighbor]:
                    ...
                    confidence_neighbor = kornia.geometry.transform.warp_affine3d(
                                ...
                                padding_mode='zeros', 
                                align_corners=True)
                    confidence_mask = (confidence_neighbor > 0.8).float()
            ...
            if len(fuse_features) > 0:
                ...
                fuse_features = ego_feature.permute(0, 2, 3, 4, 1).contiguous() #(1,Z,H,W,C)
        ...
        if self.use_asr:
            batch_fuse_features = self.asr(batch_fuse_features)
    ...
    def loss(self,
        ...
        if self.stage == 2:
            codebook_loss = preds_dicts['codebook_loss']
            loss_dict = {'loss_codebook': codebook_loss}
            return loss_dict
    ...
    def _freeze_ego(self):
        ...
        self.v2v_fuse.eval()
        for param in self.v2v_fuse.parameters():
            param.requires_grad = False
