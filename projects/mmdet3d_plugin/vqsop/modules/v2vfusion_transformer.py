...
from projects.mmdet3d_plugin.vqsop.modules.point_generator import MlvlPointGenerator
...
class V2VFusionTransformer(BaseModule):
    ...
    def get_reference_points(H, W, Z, bs=1, device='cuda', dtype=torch.float):
        ...
        ref_3d = ref_3d[None, None].repeat(bs, 1, 1, 1)#(bs,1,Z*H*W,3)
        return ref_3d
    ...
    def forward(
        ...
        reference_points = self.get_reference_points(x, y, z, bs=bs, device=volume_queries.device)#(bs,1,Z*H*W,3)
        # (h_i * w_i * d_i, 2)
        ...
        volume_queries = volume_queries.flatten(2).permute(0,2,1)#(bs,volume_h*volume_w*volume_z,C)
        query_pos = query_pos.flatten(2).permute(0,2,1)#(bs,volume_h*volume_w*volume_z,C)
        feat_flatten = []
        spatial_shapes = []
        key_pos_list=[]
        voxel_masks = []
