...
class Encoder(TransformerLayerSequence):
    ...
    def get_reference_points(H, W, Z, bs=1, device='cuda', dtype=torch.float):
        ...
        ref_3d = ref_3d.permute(3, 0, 1, 2)#(3,Z,H,W)
        ref_3d = ref_3d.flatten(1)#(3,Z*H*W)

        ref_3d = ref_3d.permute(1, 0)#(Z*H*W,3)
        ref_3d = ref_3d[None, None].repeat(bs, 1, 1, 1)
        return ref_3d#(bs,1,Z*H*W,3) 
    ...
    def point_sampling(self, reference_points, pc_range,  img_metas):
        ...
        reference_points = reference_points.permute(1, 0, 2, 3)#(1,bs,Z*H*W,3)

        D, B, num_query = reference_points.size()[:3]
     
        reference_points = reference_points.view(
            D, B, 1, num_query, 4).repeat(1, 1, self.cam_num, 1, 1).unsqueeze(-1)   # (D, B, num_cam, num_query, 4, 1)
      
        lidar2img = lidar2img.view(
            1, B, self.cam_num, 1, 4, 4).repeat(D, 1, 1, num_query, 1, 1)  #(D,B,num_cam,num_query,4,4)
        ...
        reference_points_cam = reference_points_cam.permute(2, 1, 3, 0, 4) #self.cam_num, B, num_query, D, 2
    ...
    def forward(self,
        ...
        output = volume_query#(volume_h*volume_w*volume_z,bs,C)
        intermediate = []
        ...
        reference_points_cam, volume_mask = self.point_sampling(
            ref_3d, self.pc_range, kwargs['img_metas'])#(self.cam_num, B, num_query, D, 3), (self.cam_num, B, num_query, D)

        volume_query = volume_query.permute(1, 0, 2)#(bs,volume_h*volume_w*volume_z,C)
        ...
        for lid, layer in enumerate(self.layers):
            output = layer(
                ...
                value,
                *args,
                ref_3d=ref_3d,#(bs,1,Z*H*W,3)
                volume_h=volume_h,
                volume_w=volume_w,
                volume_z=volume_z,
                spatial_shapes=spatial_shapes,
                level_start_index=level_start_index,
                reference_points_cam=reference_points_cam,#(self.cam_num, B, num_query, D, 3)
                bev_mask=volume_mask,#(self.cam_num, B, num_query, D)
                **kwargs)
        ...
        for layer in self.operation_order:
            ...
            # spaital cross attention
                ...
                query = self.attentions[attn_index](
                    query,#(bs,volume_h*volume_w*volume_z,C)
                    key,
                    value,
                    identity if self.pre_norm else None,
                    query_pos=query_pos,
                    key_pos=key_pos,
                    reference_points=ref_3d,#(bs,1,Z*H*W,3)
